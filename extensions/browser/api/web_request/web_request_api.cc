...
bool WebRequestAPI::MaybeProxyURLLoaderFactory(
  ...
  static int s_call = 0;
  ++s_call;
  LOG(INFO) << "[Afterbird/WebRequest] MaybeProxy call#" << s_call
            << " type=" << static_cast<int>(type)
            << " may_have=" << MayHaveProxies()
            << " initiator=" << request_initiator.Serialize();
  if (!MayHaveProxies()) {
...
void WebRequestAPI::OnExtensionLoaded(content::BrowserContext* browser_context,
                                      const Extension* extension) {
  const bool has_perms = HasAnyWebRequestPermissions(extension);
  LOG(INFO) << "[Afterbird/WebRequest] OnExtensionLoaded id=" << extension->id()
            << " name=\"" << extension->name()
            << "\" has_web_request_perms=" << has_perms;
  if (has_perms) {
    ++web_request_extension_count_;
    UpdateMayHaveProxies();
    LOG(INFO) << "[Afterbird/WebRequest] count=" << web_request_extension_count_
              << " may_have_proxies=" << may_have_proxies_;
  }
}
...
WebRequestInternalAddEventListenerFunction::Run() {
  ...
  LOG(INFO) << "[Afterbird/WebRequest] AddEventListener ext="
            << extension_id_safe() << " event=" << event_name
            << " sub=" << sub_event_name
            << " extra_info_spec=" << extra_info_spec;
  bool success =
      WebRequestEventRouter::Get(browser_context())
  ...
  LOG(INFO) << "[Afterbird/WebRequest] AddEventListener success=" << success;
