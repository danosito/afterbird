...
#else
// Afterbird: desktop-android builds don't compile the <webview> guest-view
// source_set. There are no webview frames on Android, so the checks below
// can unconditionally say "not a webview". Provide a header-only shim with
// an inline FromRenderFrameHost() that always returns null.
namespace extensions {
class WebViewGuest {
 public:
  static WebViewGuest* FromRenderFrameHost(content::RenderFrameHost*) {
    return nullptr;
  }
};
}  // namespace extensions
