...
const base::Value::Dict& GetAPISchema(const std::string& api_name) {
  ...
  // Afterbird: on desktop-android-extensions we only compile a subset of the
  // API schemas. If JS reaches for a chrome.* namespace whose schema we never
  // generated (e.g. chrome.developerPrivate on chrome://extensions), crashing
  // the whole renderer loses the entire tab; return an empty schema instead
  // and let the runtime report "not available" to JS callers.
  if (!schema) {
    LOG(WARNING) << "extensions: GetAPISchema miss for \"" << api_name
                 << "\"; returning empty schema (desktop-android build).";
    static const base::NoDestructor<base::Value::Dict> kEmptySchema;
    return *kEmptySchema;
  }
#else
