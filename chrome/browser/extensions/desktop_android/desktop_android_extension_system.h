...
class DesktopAndroidExtensionSystem : public ExtensionSystem {
 public:
  ...
  // Afterbird: exposed so developerPrivate handlers can enable/disable/remove
  // extensions without reaching into ExtensionService (which isn't compiled on
  // desktop-android). Returns nullptr until InitForRegularProfile runs.
  ExtensionRegistrar* extension_registrar() { return registrar_.get(); }
