...
// Afterbird: pull in the extensions WebUI header when building with the
// experimental desktop-android extensions mode (even though ENABLE_EXTENSIONS
// is off) so we can register its prefs below. Without this, opening
// chrome://extensions crashes in ExtensionsUI::ExtensionsUI because
// kExtensionsUIDeveloperMode is never registered.
...
// Afterbird: when the full extensions path is disabled but desktop-android
// extensions mode is on, we still need the ExtensionsUI preferences so that
// chrome://extensions can initialize (reading
